/**
 * Fetch the small primary-source set used to curate the reviewed radar ledger.
 *
...
const OUT = join(ROOT, "tmp", "radar", "reviewed");
...
for (const source of SOURCES) {
  try {
    ...
    manifest.documents.push({ ...source, status: "collected", transport, sha256, bytes: Buffer.byteLength(body), path: `tmp/radar/reviewed/${source.id}.html` });
...
console.log(`done: ${manifest.documents.filter((document) => document.status === "collected").length}/${SOURCES.length} reviewed sources collected`);
